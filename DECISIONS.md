...
## 2026-08-16: Renamed the product to Rubric
Renamed MCP Studio to Rubric: `mcp-rubric` (CLI, binary `mcp-rubric`), `mcp-rubric-server`, `mcp-rubric-shared`, `mcp-rubric-client` (private), all at 0.2.0. `mcp-studio` is permanently unpublishable on npm — its typosquat guard strips hyphens, colliding with the existing `mcpstudio` package — and `mcp-studio-cli` is owned by an unrelated project; a rubric is a published scoring guide of weighted criteria producing a grade, which is exactly what the scanner is. Rejected: Assay, Touchstone, Crucible, Muster (all viable, none as precise about the mechanism), plain `mcp-conformance` (discoverable but no identity), `@mcp-studio-dev/*` scope (bakes a throwaway account name into the install command). Full write-up in NAMING.md.
...
## 2026-08-15: CLI publish on hold — package name contested (superseded 2026-08-16)
